...
export default class NormRepository implements INorm {
  public async createNorm(norma: CreateNormDTO): Promise<ResponseNorm> {
    try {
      const usuarioExiste = await prisma.users.findUnique({
        where: { id_user: norma.adm_criador },
        select: { id_user: true },
      });

      if (!usuarioExiste) {
        throw new ValidatorError(
          `Usuário administrador (adm_criador=${norma.adm_criador}) não encontrado`,
          400,
        );
      }

      const admCriadorValido = norma.adm_criador;
      ...
      const norma_ = await prisma.norma.create({
        data: {
          ...
          usuario: { connect: { id_user: admCriadorValido } },
          orgaos: {
            connectOrCreate: {
              ...
              create: {
                ...
                adm_criador: norma.adm_criador, // ← direto no campo
              },
            },
          },
          normas_origem: {
  ...
  public async findByCodigo(
    norm_codigo: string,
  ): Promise<{ pdf_caminho: string } | null> {
    return await prisma.norma.findUnique({
      where: { norm_codigo },
      select: { pdf_caminho: true },
    });
  }
