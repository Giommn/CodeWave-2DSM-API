...
export default class NormRepository implements INorm {
  public async createNorm(norma: CreateNormDTO): Promise<ResponseNorm> {
    try {
      ...
      const norma_filtrada: ResponseNorm = {
        ...
        referencias: norma_.normas_origem.map(
          (ref) => ref.norma_destino.norm_titulo,
        ),
        categoria: norma_.categoria.map((cat) => cat.cat.cat_nome),
        notas: norma_.notas.map((nota) => {
          return {
            notaIT: nota.not_IT,
      ...
      console.log("ERRO REAL:", JSON.stringify(error));
      console.log("ERRO MESSAGE:", error.message);
      console.log("ERRO CODE:", error.code);
      throw new ValidatorError(
        "Could not create norm or already exists.",
        400,
        error.message,
      );
    }
  ...
  public async deleteNorm(id: number): Promise<ResponseNorm> {
    try {
      ...
      const norma_filtrada: ResponseNorm = {
        ...
        pdf_caminho: norma.pdf_caminho,
        referencias: norma.normas_origem.map(
          (ref) => ref.norma_destino.norm_titulo,
        ),
        categoria: norma.categoria.map((cat) => cat.cat.cat_nome),
        notas: norma.notas.map((nota) => {
          return {
            notaIT: nota.not_IT,
            notaTitulo: nota.not_titulo,
            notaAB: nota.not_AB,
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        return {
          ...
          emissao: normaAtualizada.emissao
          ...
          referencias: normaAtualizada.normas_origem.map(
            (ref) => ref.norma_destino.norm_titulo,
          ),
          categoria: normaAtualizada.categoria.map((cat) => cat.cat.cat_nome),
          notas: normaAtualizada.notas.map((nota) => {
            return {
              notaIT: nota.not_IT,
              notaTitulo: nota.not_titulo,
              notaAB: nota.not_AB,
      ...
      throw new ValidatorError(
        "It was not possible to update the standard",
        400,
        erro.message,
      );
    }
  ...
  public async saveNormsInHistoric(
    id_norm: number,
    id_user: number,
  ): Promise<void> {
    try {
  ...
  public async favoritarNorma(id_user: number, id_norm: number) {
    try {
      ...
      throw new ValidatorError(
        "Não foi possivel adicionar aos favoritos",
        400,
        error.message,
      );
    }
  ...
  public async tirarFavoritoNorma(
    id_user: number,
    id_norm: number,
  ): Promise<void> {
    try {
  ...
  public async pegarMinhasNormasFavoritas(
    id_user: number,
  ): Promise<Array<ResponseNorm>> {
    try {
      ...
      throw new ValidatorError(
        "Ouve um erro ao procurar pelas suas normas",
        400,
        erro.message,
      );
    }
