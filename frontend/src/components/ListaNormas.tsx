...
function ListaNormas({ arquivos, apiUrl, onNormaExcluida }: Props) {
  ...
  if (arquivos.length === 0) {
    return (
      <div className="flex items-center justify-center p-12 text-gray-400 text-lg rounded-2xl border border-dashed border-gray-300">
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-gray-200">
      ...
      {arquivos.map((norma, index) => (
        <div
          key={norma.id_norm}
          className={`flex items-center gap-4 px-4 py-3 text-sm border-t border-gray-200 transition-colors
        ...
        >
          ...
          <span className="w-28">
            <span className="border border-gray-300 text-gray-700 text-xs px-2 py-0.5 rounded-lg whitespace-nowrap">
              {norma.emissao}
            </span>
          </span>
