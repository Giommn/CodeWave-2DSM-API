...
export function ListaNormasAprovadas({
  ...
  const [normaParaVisualizar, setNormaParaVisualizar] =
    useState<ResponseNorm | null>(null);
  const [isViewModalOpen, setIsViewModalOpen] = useState(false);
  const [normaParaEditar, setNormaParaEditar] = useState<ResponseNorm | null>(
    null,
  );
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [normaParaExcluir, setNormaParaExcluir] = useState<ResponseNorm | null>(
    null,
  );
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 w-full">
        <div className="flex flex-col min-w-[1100px]">
          ...
          {arquivos.map((norma, index) => (
            ...
            >
              <div
                onClick={() => abrirVisualizacao(norma)}
              ...
              <div className="w-[5%] flex items-center justify-center pr-4 relative">
                <div
                  ref={menuAbertoId === norma.id_norm ? menuRef : null}
                  className="relative"
                >
                  <button
                    onClick={() =>
                      setMenuAbertoId(
                        menuAbertoId === norma.id_norm ? null : norma.id_norm,
                      )
                    }
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 text-center">
            ...
            <p className="text-sm text-gray-600 mb-6">
              Deseja excluir a norma{" "}
              <strong className="text-[#72203E]">
                {normaParaExcluir.norm_codigo}
              </strong>{" "}
              definitivamente?
            </p>
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-3 rounded-xl font-bold text-gray-700 border border-gray-300 hover:bg-gray-50 transition-colors"
              >
                Cancelar
              </button>
              <button
