...
export function ModalNotificacao({ 
  ...
  const abrirPdf = async (nomeOriginal: string, caminho: string) => {
    if (!caminho) {
      alert("Nenhum arquivo PDF associado a este item (caminho vazio).");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Falha ao localizar o arquivo.");
      }
      ...
      alert(
        "Não foi possível visualizar o PDF. Isso pode ocorrer porque a norma original já foi deletada " +
        "ou um pedido de alteração recente já atualizou o arquivo desta norma."
      );
    } finally {
  ...
  return (
    <>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4 py-6 overflow-y-auto">
        <div className={`bg-white rounded-xl shadow-xl w-full transition-all duration-300 relative my-auto ${isComparing ? 'max-w-6xl' : 'max-w-2xl'}`}>
            ...
            <div className="flex flex-col">
              <h2 className="text-gray-800 font-bold text-xl">
                {userRole === "user" 
                  ...
                  : `Avaliando pedido de ${pedido.tipo_pedido === 'edicao' ? 'Edição' : 'Criação'}: ${pedido.tipo_pedido === 'edicao' ? pedido.norma_nome : pedido.codigo}`}
              </h2>
          ...
          <div className={`p-6 max-h-[60vh] overflow-y-auto custom-scrollbar ${isComparing ? 'grid grid-cols-2 gap-8 bg-gray-50' : 'flex flex-col gap-4'}`}>
            ...
            {isComparing && (
              <div className="flex flex-col gap-4 border-r border-gray-300 pr-8">
                ...
                {pedido.versao_anterior && pedido.versao_anterior.titulo !== "Norma não encontrada no sistema" ? (
                  <>
                    <div className="bg-gray-100 rounded-lg p-4 flex flex-col items-center justify-center border border-gray-300">
                      ...
                      <button 
                        ...
                        className="bg-white border border-gray-400 text-gray-700 px-4 py-1.5 rounded text-xs font-bold hover:bg-gray-50 transition-colors w-full disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isCarregandoPdf ? "Carregando..." : "Abrir PDF Atual"}
                      </button>
                    </div>

                    <div><label className="block text-xs font-bold text-gray-500 mb-1">Código</label><input type="text" disabled value={pedido.versao_anterior.codigo} className="w-full border border-gray-300 rounded-md p-2 bg-gray-200 text-gray-600" /></div>
                    ...
                    <div><label className="block text-xs font-bold text-gray-500 mb-1">Descrição:</label><textarea disabled value={pedido.versao_anterior.descricao} rows={3} className="w-full border border-gray-300 rounded-md p-2 bg-gray-200 text-gray-600 resize-none"></textarea></div>
                  </>
                ) : (
                  <div className="flex flex-col items-center justify-center text-center p-6 bg-gray-100 border border-dashed border-gray-300 rounded-lg h-full">
                    <IoDocumentTextOutline className="w-10 h-10 text-gray-400 mb-2" />
                    <span className="text-gray-500 text-sm font-medium">Os dados da norma atual não foram encontrados. Ela pode ter sido excluída ou substituída.</span>
                  </div>
                )}
              </div>
            )}

            {/* LADO DIREITO (OU ÚNICO): PEDIDO NOVO */}
            <div className={`flex flex-col gap-4 ${isComparing ? 'bg-white p-6 rounded-lg shadow-sm border border-gray-300' : ''}`}>
              ...
              <div className="bg-gray-50 rounded-lg p-6 flex flex-col items-center justify-center border border-dashed border-gray-300">
                ...
                <button 
                  ...
                  disabled={isCarregandoPdf}
                  className="mt-4 bg-[#72203E] text-white px-5 py-2 rounded-md text-sm font-bold hover:bg-[#5a1931] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isCarregandoPdf ? "Carregando..." : "Visualizar Proposta no Visualizador"}
                </button>
              </div>

              <div><label className="block text-sm font-bold text-gray-700 mb-1">Código</label><input type="text" disabled value={pedido.codigo} className="w-full border border-gray-300 rounded-md p-2 bg-gray-50 text-gray-800 font-medium" /></div>
              ...
              <div><label className="block text-sm font-bold text-gray-700 mb-1">Descrição:</label><textarea disabled value={pedido.descricao} rows={3} className="w-full border border-gray-300 rounded-md p-2 bg-gray-50 text-gray-800 font-medium resize-none"></textarea></div>
              
              {/* Categoria Oculta em caso de Edição */}
              {pedido.tipo_pedido === 'criacao' && (
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Categoria:</label>
                  <input type="text" disabled value={pedido.categoria} className="w-full border border-gray-300 rounded-md p-2 bg-gray-50 text-gray-800 font-medium" />
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[10010] flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-md relative flex flex-col gap-4 border-t-4 border-[#72203E]">
            ...
            <button 
              ...
              disabled={isCarregandoPdf}
              className="flex items-center justify-center gap-2 w-full bg-white text-gray-700 border border-gray-300 font-bold py-2 rounded hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <IoEyeOutline className="w-5 h-5" />
              {isCarregandoPdf ? "Carregando..." : "Visualizar PDF Oficial"}
            </button>
          </div>
        </div>
      )}
