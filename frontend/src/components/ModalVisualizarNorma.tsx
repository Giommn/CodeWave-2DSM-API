...
export function ModalVisualizarNorma({ isOpen, onClose, norma, onAbrirNormaAssociada }: ModalVisualizarNormaProps) {
  ...
  // Ajuste para garantir que as normas relacionadas apareçam independente do nome do campo no JSON
          ...
          <div className="p-6 max-h-[65vh] overflow-y-auto custom-scrollbar flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input type="text" disabled value={norma.norm_titulo} className="w-full border border-gray-200 rounded-md p-2 bg-gray-100 text-gray-600 font-medium cursor-not-allowed" />
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Data de Emissão:</label>
                <input type="text" disabled value={norma.emissao || "Não definida"} className="w-full border border-gray-200 rounded-md p-2 bg-gray-100 text-gray-600 font-medium cursor-not-allowed" />
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Descrição:</label>
                <textarea disabled value={norma.norm_desc} rows={2} className="w-full border border-gray-200 rounded-md p-2 bg-gray-100 text-gray-600 font-medium resize-none cursor-not-allowed"></textarea>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Categoria:</label>
                <input type="text" disabled value={Array.isArray(norma.categoria) ? norma.categoria.join(", ") : (norma.categoria || "Não definida")} className="w-full border border-gray-200 rounded-md p-2 bg-gray-100 text-gray-600 font-medium cursor-not-allowed" />
              </div>
              
              {/* CAMPO DE NORMAS ASSOCIADAS (CORRIGIDO) */}
              <div>
